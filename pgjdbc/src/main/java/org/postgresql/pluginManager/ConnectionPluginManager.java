...
public class ConnectionPluginManager {
  ...
  private static final transient Logger LOGGER = Logger.getLogger(ConnectionPluginManager.class.getName());
  private static final String ALL_METHODS = "*";
  private static final String OPEN_INITIAL_CONNECTION_METHOD = "openInitialConnection";

  protected Properties props = new Properties();
  protected ArrayList<IConnectionPlugin> plugins;
  protected ICurrentConnectionProvider currentConnectionProvider;
   ...
   *
   */
  public void init(ICurrentConnectionProvider currentConnectionProvider, Properties props) throws PSQLException {
    instances.add(this);
    ...
    if (!Util.isNullOrEmpty(factoryClazzNames)) {
      try {
        IConnectionPluginFactory[] factories = Util.<IConnectionPluginFactory>loadClasses(
              factoryClazzNames,
  ...
  public void openInitialConnection(HostSpec[] hostSpecs, Properties props, String url) throws SQLException {
  ...
  protected void openInitialConnectionOneLevel(int pluginIndex,
    ...
    if (pluginIndex == this.plugins.size()) {
      ...
      func = () -> {
        openInitialConnectionOneLevel(nextPluginIndex, hostSpecs, props, url, openInitialConnectionFunc);
        return null;
      };
    }
  ...
  protected <T> T executeOneLevel(int pluginIndex,
    ...
    do {
      plugin = this.plugins.get(pluginIndex);
      Set<String> pluginSubscribedMethods = plugin.getSubscribedMethods();
      isSubscribed = pluginSubscribedMethods.contains(ALL_METHODS) || pluginSubscribedMethods.contains(methodName);
      pluginIndex++;
    } while (!isSubscribed && pluginIndex <= this.plugins.size() - 1);
  ...
  public void releaseResources() {
    ...
    // This step allows all connection plugins a chance to clean up any dangling resources or perform any
    // last tasks before shutting down.
    this.plugins.forEach(IConnectionPlugin::releaseResources);
  }
