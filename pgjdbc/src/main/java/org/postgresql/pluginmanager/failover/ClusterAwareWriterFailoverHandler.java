 ...
 * writer may be disrupted either by temporary network issue, or due to writer host
...
public class ClusterAwareWriterFailoverHandler implements WriterFailoverHandler {
   ...
   * ClusterAwareWriterFailoverHandler constructor.
   *
   * @param topologyService           The topology service used to update and query the cluster
   *                                  topology
   * @param connectionProvider        The connection provider used to establish the database
   *                                  connection
   * @param connectionProps           a set of properties for a new connections
   * @param readerFailoverHandler     The reader failover handler used with this connection
   ...
   * @param reconnectWriterIntervalMs The time in ms to wait before re-attempting to reconnect to
   *                                  the writer
   */
  public ClusterAwareWriterFailoverHandler(
      ITopologyService topologyService,
  ...
  public WriterFailoverResult failover(List<HostSpec> currentTopology) throws SQLException {
    if (currentTopology.isEmpty()) {
      LOGGER.log(Level.FINE,
          "[ClusterAwareWriterFailoverHandler] failover was called with an invalid (empty) "
              + "topology");
    ...
    try {
      ...
      LOGGER.log(Level.FINE,
          "[ClusterAwareWriterFailoverHandler] Failed to connect to the writer instance.");
      return new WriterFailoverResult(false, false, new ArrayList<>(), null);
    } finally {
   ...
   * @param executorService   The {@link ExecutorService} that manages the task execution, which
   *                          will
   ...
   */
  private void submitTasks(List<HostSpec> currentTopology, ExecutorService executorService,
      CompletionService<WriterFailoverResult> completionService) {
  ...
  /**
   * Poll the {@link CompletionService} passed in for the next completed task and process the
   * results
   *
   ...
   *               {@link WriterFailoverResult} for which
   *               {@link WriterFailoverResult#isConnected()} returns
   *               true.
   */
  private void logTaskSuccess(WriterFailoverResult result) {
    @Nullable List<HostSpec> topology = result.getTopology();
    ...
    HostSpec newWriterHost = topology.get(WRITER_CONNECTION_INDEX);
    String newWriterHostPair =
        newWriterHost == null ? "<null>" : FailoverUtil.getHostPortPair(newWriterHost);
    String message;
    if (result.isNewHost()) {
      message =
          "[ClusterAwareWriterFailoverHandler] Successfully connected to the new writer instance:"
              + " ''{0}''";
    } else {
      message =
          "[ClusterAwareWriterFailoverHandler] Successfully re-connected to the current writer "
              + "instance: ''{0}''";
    }
  ...
  private class ReconnectToWriterHandler implements Callable<WriterFailoverResult> {
    ...
    ReconnectToWriterHandler(HostSpec originalWriterHost) {
    ...
    public WriterFailoverResult call() {
      LOGGER.log(Level.FINE,
          ...
          FailoverUtil.getHostPortPair(originalWriterHost));
      try {
        while (true) {
          try {
            BaseConnection conn = connectionProvider.connect(
                new HostSpec[]{this.originalWriterHost},
                connectionProps,
                FailoverUtil.getUrl(this.originalWriterHost, connectionProps));

            List<HostSpec> latestTopology = topologyService.getTopology(conn, true);
        ...
        LOGGER.log(Level.WARNING,
            "[ClusterAwareWriterFailoverHandler] [TaskA] encountered an exception: {0}", ex);
        throw ex;
      } finally {
     ...
     *
     * @param latestTopology The latest topology that was fetched by the newly established
     *                       connection
    ...
    private boolean isCurrentHostWriter(List<HostSpec> latestTopology) {
      String currentWriterName = this.originalWriterHost.getHost();
      HostSpec latestWriter = latestTopology.get(WRITER_CONNECTION_INDEX);
      if (currentWriterName == null || latestWriter == null) {
        return false;
      }

      String latestWriterName = latestWriter.getHost();
  ...
  private class WaitForNewWriterHandler implements Callable<WriterFailoverResult> {
    ...
    WaitForNewWriterHandler(List<HostSpec> currentTopology, @Nullable HostSpec originalWriterHost) {
      this.currentTopology = currentTopology;
      this.originalWriterHost = originalWriterHost;
    }

    /**
     * This task will repeatedly attempt to connect to a reader host in order to check the new
     * topology,
     * and then connect to the newly promoted writer
     */
    public WriterFailoverResult call() {
      LOGGER.log(Level.FINE,
          "[ClusterAwareWriterFailoverHandler] [TaskB] Attempting to connect to a new writer "
              + "instance");
      ...
      try {
        ...
        LOGGER.log(Level.WARNING,
            "[ClusterAwareWriterFailoverHandler] [TaskB] encountered an exception: {0}", ex);
        throw ex;
      } finally {
        performFinalCleanup();
      }
    ...
    private void connectToReader() throws InterruptedException {
      while (true) {
        try {
          ...
          if (connResult.isConnected()
            ...
            if (this.currentReaderHost != null) {
              hostPortPair = FailoverUtil.getHostPortPair(currentReaderHost);
            }
            LOGGER.log(Level.FINE,
        ...
        LOGGER.log(Level.FINER,
            "[ClusterAwareWriterFailoverHandler] [TaskB] Failed to connect to any reader.");
    ...
    private boolean refreshTopologyAndConnectToNewWriter() throws InterruptedException {
      while (this.currentReaderConnection != null) {
        List<HostSpec> topology = topologyService.getTopology(this.currentReaderConnection, true);
        if (!topology.isEmpty()) {
    ...
    /**
     * Check that the writer in the topology is a new host and not the original failed host
     * caused us
     * to initialize failover. If it is a new host, connect to it. The other scenario will be
     * handled
     * by the {@link ReconnectToWriterHandler}
     *
     * @param writerCandidate The {@link HostSpec} for the writer to potentially connect to
    ...
    private boolean connectToWriter(HostSpec writerCandidate) {
      try {
        LOGGER.log(Level.FINER,
            ...
            FailoverUtil.getHostPortPair(writerCandidate));
        ...
        if (isSame(writerCandidate, this.currentReaderHost)) {
          ...
          this.currentConnection = connectionProvider.connect(
              new HostSpec[]{writerCandidate},
              connectionProps,
              FailoverUtil.getUrl(writerCandidate, connectionProps));
        }
     ...
     */
    private boolean isSame(HostSpec writerCandidate, @Nullable HostSpec originalWriter) {
      if (writerCandidate == null || originalWriter == null) {
        return false;
      }
      String writerCandidateName = writerCandidate.getHost();
      return writerCandidateName != null && writerCandidateName.equals(originalWriter.getHost());
    }
