...
public class AuroraTopologyService implements ITopologyService {
  ...
  static final String RETRIEVE_TOPOLOGY_SQL =
      "SELECT SERVER_ID, SESSION_ID FROM aurora_replica_status() "
          ...
          + "WHERE EXTRACT(EPOCH FROM(NOW() - LAST_UPDATE_TIMESTAMP)) <= 300 OR SESSION_ID = "
          + "'MASTER_SESSION_ID' "
  ...
  public AuroraTopologyService(int refreshRateInMilliseconds) {
    ...
    this.clusterInstanceTemplate = new HostSpec("?", NO_PORT);
  }
   ...
   * @param gatherMetrics The metric settings that decides whether the class should gather
   *                      metrics
   */
   ...
   */
  @Override
  public void setClusterInstanceTemplate(HostSpec clusterInstanceTemplate) {
    LOGGER.log(Level.FINER,
        "[AuroraTopologyService] clusterInstance host=''{0}'', port={1,number,#}",
   ...
   */
  @Override
  public List<HostSpec> getTopology(Connection conn, boolean forceUpdate) {
    ...
    if (clusterTopologyInfo == null
      ...
      if (!latestTopologyInfo.hosts.isEmpty()) {
        ...
        return (clusterTopologyInfo == null || forceUpdate) ? new ArrayList<>()
  ...
  /**
   * Checks whether it is necessary to refresh and update the topology.
   *
   * @param info ClusterTopologyInfo instance that contains information about the topology including
   *             the last updated time
   * @return True if the topology is empty (may still need to be initialized) or if the time
   * since the
  ...
  protected ClusterTopologyInfo queryForTopology(Connection conn) {
    ...
    try (Statement stmt = conn.createStatement()) {
      ...
      // Intentionally ignore this SQLException
    }
   ...
   * @return A list of {@link HostSpec} objects representing the topology that was returned by
   * the topology query. The
  ...
  private List<HostSpec> processQueryResults(ResultSet resultSet) throws SQLException {
    ...
    while (resultSet.next()) {
      ...
      if (writerCount == 0) {
        ...
        // updated timestamp, this host should be the obsolete writer, and it is about to become
        // a reader
    ...
    if (writerCount == 0) {
      LOGGER.log(Level.SEVERE,
          "[AuroraTopologyService] The topology query returned an invalid topology - no writer "
              + "instance detected");
      hosts.clear();
    }
    return hosts;
  ...
  private HostSpec createHost(ResultSet resultSet, boolean isWriter) throws SQLException {
    ...
    return new HostSpec(getHostEndpoint(hostName), this.clusterInstanceTemplate.getPort());
  }
  ...
  /**
   * Store the information for the topology in the cache, creating the information object if it
   * did not previously exist
   * in the cache
   *
   * @param clusterTopologyInfo The cluster topology info that existed in the cache before the
   *                            topology query. This parameter
   *                            will be null if no topology info for the cluster has been created
   *                            in the cache yet.
   ...
   */
  private ClusterTopologyInfo updateCache(@Nullable ClusterTopologyInfo clusterTopologyInfo,
   ...
   */
  @Override
  public @Nullable HostSpec getLastUsedReaderHost() {
    ClusterTopologyInfo info = topologyCache.get(this.clusterId);
   ...
   */
  @Override
  public void setLastUsedReaderHost(@Nullable HostSpec reader) {
    if (reader != null) {
      synchronized (cacheLock) {
        ClusterTopologyInfo info = topologyCache.get(this.clusterId);
        if (info != null) {
   ...
   */
  @Override
  public void addToDownHostList(@Nullable HostSpec downHost) {
    if (downHost == null) {
      return;
    }
    ...
    synchronized (cacheLock) {
      ...
      if (clusterTopologyInfo == null) {
        clusterTopologyInfo =
            new ClusterTopologyInfo(new ArrayList<>(), new HashSet<>(), null, Instant.now());
        topologyCache.put(this.clusterId, clusterTopologyInfo);
      }
      clusterTopologyInfo.downHosts.add(FailoverUtil.getHostPortPair(downHost));
    }
  }
   ...
   *
   * @param host The {@link HostSpec} object representing the host to remove from the list of
   *             down hosts
   */
  @Override
  public void removeFromDownHostList(@Nullable HostSpec host) {
    if (host == null) {
      return;
    }
    synchronized (cacheLock) {
      ClusterTopologyInfo clusterTopologyInfo = topologyCache.get(this.clusterId);
      if (clusterTopologyInfo != null) {
        clusterTopologyInfo.downHosts.remove(FailoverUtil.getHostPortPair(host));
      }
    }
  }
  ...
  static class ClusterTopologyInfo {
     ...
     */
    ClusterTopologyInfo(List<HostSpec> hosts, Set<String> downHosts,
        @Nullable HostSpec lastUsedReader, Instant lastUpdated) {
      this.hosts = hosts;
