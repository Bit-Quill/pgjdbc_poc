...
public class DefaultMonitorService implements IMonitorService {
  ...
  private static final transient Logger LOGGER = Logger.getLogger(DefaultMonitorService.class.getName());
  ...
  public DefaultMonitorService() {
    this((hostInfo, props, monitorService) -> {
      ...
      int monitorDisposalTime = Integer.parseInt(PGProperty.MONITOR_DISPOSAL_TIME.getDefaultValue());
      try {
        monitorDisposalTime = PGProperty.MONITOR_DISPOSAL_TIME.getInt(props);
      } catch (PSQLException ex) {
        LOGGER.warning("Invalid value for property ''monitorDisposalTime''. Use default value instead.");
      }
  ...
  public MonitorConnectionContext startMonitoring(
    ...
    if (nodeKeys.isEmpty()) {
      throw new IllegalArgumentException("Empty NodeKey set passed into DefaultMonitorService. Set should not be empty.");
    }
  ...
  public void stopMonitoring(MonitorConnectionContext context) {
    ...
    if (node == null) {
      LOGGER.warning("Invalid context passed into DefaultMonitorService. Could not find any NodeKey from context.");
      return;
    }
  ...
  public void stopMonitoringForAllConnections(Set<String> nodeKeys) {
    ...
    if (node == null) {
      LOGGER.warning("Invalid node key passed into DefaultMonitorService. No existing monitor for the given set of node keys.");
      return;
    }
  ...
  protected IMonitor getMonitor(Set<String> nodeKeys, HostSpec hostSpec, Properties props) {
    return this.threadContainer.getOrCreateMonitor(nodeKeys, () -> monitorInitializer.createMonitor(hostSpec, props, this));
  }
}
